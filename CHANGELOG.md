...
5. Added support for ArcGIS Server Info log entries (ArcGIS Server File System only); Log Level Info log entries breakdown the request performance at the framework-level and includes times for request that are not just ArcSOC-based (e.g., hosted feature services); these performance times are close to a "response time" (minus an Web-tier overhead and transport time overhead); captured times at this level may appear higher which is because the instance creation time, wait time and elapsed time durations are all included in the value; ArcGIS Server 11.3 or higher recommended for using System Log Parser Info Analysis Type; The Info Analysis Type report is based on the Optimized report logic
